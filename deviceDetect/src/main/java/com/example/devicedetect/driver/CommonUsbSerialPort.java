...
public abstract class CommonUsbSerialPort implements UsbSerialPort {
    ...
    protected void testConnection() throws IOException {
        ...
        if (len < 0) throw new IOException("USB get_status request failed");
    }
    ...
    protected int read(final byte[] dest, final int timeout, boolean testConnection) throws IOException {
        ...
        if (timeout != 0) {
            ...
            //nread = mUsbRequest.queue(ByteBuffer.wrap(dest));
    ...
    public void write(final byte[] src, final int timeout) throws IOException {
        Log.w("TimeLog", "Start Def. Write : " + System.currentTimeMillis());
        int offset = 0;
        ...
        while (offset < src.length) {
            ...
            if (true) {
                ...
                Log.w("TimeLog", "Inside Debug : " + System.currentTimeMillis() + " : " + actualLength + "/" + requestLength + " offset " + offset + "/" + src.length + " timeout " + requestTimeout);
            }
            if (actualLength <= 0) {
            ...
            offset += actualLength;
            Log.w("TimeLog", "End Def. Write" + System.currentTimeMillis());
        }
    }
